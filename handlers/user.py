...
async def web_app_handle(message: types.Message, state: FSMContext, bot: Bot):
    code = message.web_app_data.data.strip()
    # Fake the message text for reuse of process_code
...
async def cmd_start(message: types.Message, state: FSMContext, bot: Bot):
    ...
    text = (
        "<b>O'zingizga qulay tilni tanlang 🇺🇿</b>\n\n"
        "<b>Ўзингизga қулай тилни танланг 🇺🇿</b>\n\n"
...
async def process_search_name(message: types.Message, state: FSMContext):
    ...
    if not results:
        ...
        for code, title in results:
            res_list += f"• <code>{code}</code> - {html.escape(title)}\n"
